...
new_handler = '''  const handleFileSelect = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    const parseData = (data: any[]) => {
      ...
      })).filter(row => row.name && row.price); // require at least name and price
...
# 4. Add the preview table in JSX and replace onChange
...
preview_table = '''                    {parsedBulkData.length > 0 && (
                    ...
                    <button'''
content = content.replace('<button', preview_table)
# Remove the old form layout classes to fit the table better
